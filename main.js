...
function addPost(title, author, content, category) {
...
function updatePost(id, newTitle, newAuthor, newContent, newCategory) {
    let posts = getPosts();
    ...
    if (newCategory) posts[index].category = newCategory;
    savePosts(posts);
    return true;
}
    ...
    for (let post of posts) {
        ...
        html += `  
            <div class="card" data-id="${post.id}">
                ...
                <div class="card-category">${escapeHtml(post.category || 'Без рубрики')}</div>
                <div class="card-vievs">👁${post.views || 0} просмотров</div>
                ...
                <div class="card-buttons">
                    ...
                    <button class="delete-btn" data-id="${post.id}"><span class="delete-icon">🗑</span>Удалить</button>
                </div>
            </div>
        `;
    }
...
function openCreateModal() {
    ...
    const catSelect = document.getElementById('storyCategoru');
    if (catSelect) catSelect.value = 'psychologist';
}
...
function openEditModal(id) {
    ...
    if (catSelect) catSelect.value = post.category || 'psychologist';
}
...
function saveFromModal() {
    ...
    const category = document.getElementById('storyCategory')?.value || 'psychologist';
    if (currentEditId === null) {
        addPost(title, author, content, category);
    } else {
        updatePost(currentEditId, title, author, content, category);
    }
    closeModal();
    renderAllCards();
}
...
function showStoryFromURL() {
    ...
    if (view === 'story') {
        ...
        if (titleParam && authorParam) {
            let posts = getPosts();
            const postIndex = posts.findIndex(p => p.title === decodeURIComponent(titleParam) && p.author === decodeURIComponent(authorParam));
            if (postIndex !== -1) {
        ...
        const grid = document.getElementById('cardsGrid');
        // Добавляем класс для стилизации просмотра истории
        document.body.classList.add('story-view-mode');
        ...
        grid.innerHTML = `  
            <div class="story-card" style="background:white; border-radius:24px; padding:28px; width:100%; max-width:600px; box-sizing:border-box; margin:0 auto;">
        ...
        // Для компьютера: ширина карточки 70% через медиа-запрос в CSS
        // Уже добавлено правило в CSS: .story-card { max-width: 70%; } при ширине >768px
